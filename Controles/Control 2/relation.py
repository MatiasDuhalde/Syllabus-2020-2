...
class Relation:
    ...
    def insert_tuple(self, tup):
        ...
        # reset self.current_page
        self.open()

        # Check if tuple exists in relation
        while self.has_next():
            current_tuple = self.next()
            # If tuple exists, do not insert
            if current_tuple == tup:
                print(f"Can't insert tuple ({tup}), value is already present in {self.rName}")
                self.close()
                return

        # Could not find tuple, find first empty space
        # Get root page
        current_page = self.buffer.get_page(self.root_page)
        current_index = 1
        # Check if root page has empty space
        empty_index = current_page.has_empty()
        while empty_index == -1:
            # If it doesn't, check next page
            current_index += 1
            # If there isn't another page, create one
            if current_page.next == '<NULL>':
                new_page = self.buffer.create_page(self.rName + f'-{current_index}')
                # Link new page
                current_page.set_next_page(new_page.pname)
                # Save old page (<NEXT> value)
                self.buffer.evict_page(current_page.pname)
                current_page = new_page
            else:
                # Get next page
                current_page = self.buffer.get_page(self.rName + f'-{current_index}')
            # Check if current page has empty space
            empty_index = current_page.has_empty()
        
        # Insert tuple in found space
        current_page.insert_into_pos(empty_index, tup)
        # Save page
...
class RelationIterator:
    ...
    def open(self):
        return self.__relation.open()

    def __next__(self):
        tup = self.__relation.next()
        if tup == None:
            raise StopIteration
        return tup
    ...
    def has_next(self):
        return self.__relation.has_next()

    def close(self):
        return self.__relation.close()
...
if __name__ == "__main__":
    ...
    R.insert_tuple('15 12') # New tuple
